...
describe('Starting tests for AirlineSystem', function() {
  ...
  it('website title should be AirlineSystem', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('AirlineSystem');
    })
  });

  it('network-name should be airlinev9@0.0.7',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('airlinev9@0.0.7.bna');
    });
  });

  it('navbar-brand should be AirlineSystem',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('AirlineSystem');
    });
  });
